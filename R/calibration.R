...
#'
#' @param type Currently, two ways are offered. With "fast", a look-up is conducted for each calendar year that picks up the amplitude of the nearest CRA age. With "full", each CRA is calibrated individually and then summed (much slower).
...
calibrate.UncalGrid <- function(x, errors=0, calCurves='intcal13', timeRange=c(50000,0), eps=1e-5, type="fast", datenormalised=FALSE, spdnormalised=FALSE, verbose=TRUE, ...){
    ...
    if (type=="full"){
        caleach <- calibrate(x=x$CRA, errors=errors, normalised=datenormalised,...)

...
#'     \item{\strong{Statistical Test Functions}}{\code{\link{modelTest}} compares the observed SPD against a variety of theorethical models (most typically an exponential curve) using the Monte-Carlo approach; \code{\link{permTest}} compares two or more SPDs and test for the null hypothesis that all sets are derived from the same population; \code{\link{SPpermTest}} identifies, for defined intervals, locations with significantly higher/lower growth rate in the SPD  compared to the panregional trend in the data}
#' }
#' @note
